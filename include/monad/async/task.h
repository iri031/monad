...
extern "C"
{
  ...
  MONAD_ASYNC_NODISCARD extern monad_async_result monad_async_task_create(monad_async_task *task, struct monad_async_task_attr *attr);

  //! \brief EXPENSIVE Destroys a task instance.
  MONAD_ASYNC_NODISCARD extern monad_async_result monad_async_task_destroy(monad_async_task task);
  ...
  MONAD_ASYNC_NODISCARD extern monad_async_result monad_async_task_attach(monad_async_executor executor, monad_async_task task); // implemented in executor.c

  //! \brief Suspend execution of a task for a given duration, which can be zero (which equates "yield").
  MONAD_ASYNC_NODISCARD extern monad_async_result monad_async_task_suspend_for_duration(monad_async_task task, uint64_t ns); // implemented in executor.c
