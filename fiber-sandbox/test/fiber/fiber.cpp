...
int test = 0;
monad_fiber_context_t * spawn_test(void * ptr, monad_fiber_context_t * me, monad_fiber_context_t * from)
{
...
TEST(fiber, spawn)
{
  ...
  test = 0;
  auto f = monad_fiber_context_callcc(monad_fiber_main_context(),
                                      monad_fiber_default_stack_size, false, &spawn_test, &test);
  EXPECT_EQ(test, 1);
  EXPECT_NE(f, nullptr);
  f = monad_fiber_context_switch(monad_fiber_main_context(), f); // complete it
  EXPECT_EQ(test, 2);
  EXPECT_EQ(f, nullptr);
}
