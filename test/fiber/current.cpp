...
TEST(current, yield_to_task)
{
  ...
  struct my_task : monad_fiber_task
  {
    ...
    my_task() :
      monad_fiber_task{
          .resume=+[](monad_fiber_task * this_){static_cast<my_task*>(this_)->resume(); },
          .destroy=+[](monad_fiber_task * ){ASSERT_EQ("Musten't be reached", nullptr);}
      }
    {}
  ...
  monad_fiber_yield(); // this should run one, since it's the same priority and we're yielding
  EXPECT_TRUE(t3.ran);
...
{
  std::thread{
    [us = reinterpret_cast<std::uintptr_t>(us_), to_resume]
    {
      usleep(static_cast<useconds_t>(us));
      monad_fiber_scheduler_dispatch(to_resume->scheduler, &to_resume->task, to_resume->priority);
    }
  }.detach();
};
...
TEST(current, await_from_thread)
{
  ...
  mft.context = monad_fiber_context_callcc(
      monad_fiber_main()->context, 4096, false,
      +[](void * ptr, monad_fiber_context_t * fb, monad_fiber_context_t * from) -> monad_fiber_context_t *
      {
        return (*static_cast<decltype(l)*>(ptr))(fb, from);
      }, &l);
