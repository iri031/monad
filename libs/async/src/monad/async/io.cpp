...
void AsyncIO::submit_request_(timed_invocation_state *state, void *uring_data)
{
    ...
    if (capture_io_latencies_) {
        ((erased_connected_operation *)uring_data)->initiated =
...
monad_c_result AsyncIO::dispatch_task_impl_(monad_context_task task_) noexcept
{
    ...
    uint64_t suspend_for_duration = monad_async_duration_infinite_cancelling;
    for (;;) {
        ...
        if (completed != nullptr) {
            ...
            auto r = to_result(monad_async_task_suspend_for_duration(
                &completed, task, suspend_for_duration));
            if (!r) {
            ...
            // The preceding cancelling infinite wait will still be active, so
            // don't initiate more of those as the first one will do
            suspend_for_duration = monad_async_duration_infinite_non_cancelling;
        }
    }
}
