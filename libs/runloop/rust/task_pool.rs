...
unsafe impl Send for MonadAsyncIoRef {}
unsafe impl Sync for MonadAsyncIoRef {}
...
impl MonadAsyncIoRef {
    ...
    /// \brief Initiate cancellation of this i/o. Returns true if cancelled immediately.
    pub fn initiate_cancel(&mut self) -> MonadCResult<bool> {
        let iostatus = self.iostatus();
        match unsafe {
            assert_eq!(monad_async_is_io_in_progress(iostatus), true);
            to_result(monad_async_task_io_cancel(iostatus))
        } {
            Ok(_) => Ok(true),
            Err(err) => {
                if unsafe {
    ...
    /// \brief Set the waker for this i/o (more efficient than vectoring through Future)
    pub fn set_waker(&mut self, waker: Waker) {
        self.inner.as_mut().unwrap().waker = Some(waker);
    }
}
...
impl MonadAsyncTaskPoolInner {
    ...
    fn get_sleeping_task(&mut self) -> MonadCResult<monad_async_task_ptr> {
        NEED TO FILTER OUT CANCELLED TASKS HERE;
...
#[must_use]
pub struct VecOfResults {
    inner: Vec<MonadCResult<isize>>,
}

impl VecOfResults {
    pub fn sum_results(&self) -> MonadCResult<()> {
        for n in 0..self.inner.len() {
            self.inner[n]?;
        }
        Ok(())
    }
...
impl MonadAsyncTaskPool {
    ...
    fn dispatch_task_impl(&mut self, task: monad_async_task) -> monad_c_result {
        ...
        loop {
            ...
            if completed != std::ptr::null_mut() {
                println!(
                    "   Dispatcher task completed i/o contains {:?}",
                    to_result(unsafe { *(*completed).__bindgen_anon_1.result.as_ref() })
                );
    ...
    pub fn new(
        ...
        unsafe {
            (*switcher_ptr).disable_calling_thread_is_correct_checks =
                ex_attr.disable_calling_thread_is_correct_checks;
        }
        let mut ret = MonadAsyncTaskPoolInner {
            ex_ptr: ex,
    ...
    /// \brief Get the underlying C executor pointer
    pub fn underlying_C_executor(&self) -> monad_async_executor {
        self.inner.borrow().ex_ptr.head
    }
    ...
    pub fn join(&self, tasks: &[MonadAsyncTask]) -> MonadCResult<VecOfResults> {
        ...
        loop {
            ...
            if done {
                return Ok(VecOfResults {
                    inner: states.into_iter().map(|res| res.unwrap()).collect(),
                });
            }
