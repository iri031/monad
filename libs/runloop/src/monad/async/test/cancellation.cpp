            ...
            try {
                    ...
                    .value();
                {
                    ...
                    if (!r) {
                        if (r.assume_error() == errc::operation_canceled) {
                            // Cancel the connect
                            r = to_result(monad_async_task_io_cancel(&status));
                            for (;;) {
                                if (done) {
                    ...
                    */
                    ...
                    if (completed == nullptr) {
                        std::cerr << "NOTE: Task " << task
                                  ...
                                  << std::endl;
                        for (auto &i : iostatuses) {
                            r = to_result(monad_async_task_io_cancel(&i.first));
                        }
                        while (empty_completions_non_cancelling())
                            ;
                        MONAD_ASSERT(
                ...
                for (;;) {
                    ...
                    if (n < 0) {
                        ...
                        for (auto &i : iostatuses) {
                            auto r =
                                to_result(monad_async_task_io_cancel(&i.first));
                        }
                        if (done) {
                            std::cout << time(nullptr) << ": Task " << task
                        ...
                        while (monad_async_io_in_progress(status, 2) > 0) {
                            ...
                            if (!r) {
                                if (monad_async_is_io_in_progress(&status[0])) {
                                    auto r = to_result(
                                        monad_async_task_io_cancel(&status[0]));
                                }
                                if (monad_async_is_io_in_progress(&status[1])) {
                                    auto r = to_result(
                                        monad_async_task_io_cancel(&status[1]));
                                }
                                break;
                            }
                        }
