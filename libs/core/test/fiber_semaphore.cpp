...
namespace
{
    ...
    {
        auto *const sem = std::bit_cast<monad_fiber_semaphore_t *>(arg0);
        std::uint64_t count = 0;
        while (true) {
            monad_fiber_semaphore_acquire(sem, MONAD_FIBER_PRIO_NO_CHANGE);
            monad_fiber_yield(count++);
        }
        return 0;
    }
...
TEST(fiber_semaphore, serial_basic)
{
    ...
    monad_fiber_semaphore_init(
        &sem, monad::make_srcloc(std::source_location::current()));
    ASSERT_EQ(0, monad_fiber_semaphore_set_name(&sem, "sem_unit_test"));
    ...
    ASSERT_EQ(0, monad_fiber_run(consumer_fiber, &suspend_info));
    ASSERT_EQ(MF_SUSPEND_YIELD, suspend_info.suspend_type);
    ASSERT_EQ(1, suspend_info.eval);

    // As above, for the second message
    ASSERT_EQ(consumer_fiber, monad_run_queue_try_pop(run_queue));
    ASSERT_EQ(0, monad_fiber_run(consumer_fiber, &suspend_info));
    ASSERT_EQ(MF_SUSPEND_YIELD, suspend_info.suspend_type);
    ASSERT_EQ(2, suspend_info.eval);
