...
static uintptr_t dummy_function(uintptr_t)
{
    return 0;
}
...
TEST(run_queue, basic_test)
{
    ...
    for (std::size_t i = 0; i < NumberOfFibers; ++i) {
        ...
        ASSERT_EQ(0, monad_fiber_set_function(fiber, prio, dummy_function, 0));
    }
    ...
    for (std::size_t i = 0; i < NumberOfFibers; ++i) {
        ...
        ASSERT_EQ(
            ...
            monad_fiber_set_function(
                fiber, MONAD_FIBER_PRIO_HIGHEST, dummy_function, 0));
    }
    ASSERT_EQ(0, monad_run_queue_create(nullptr, NumberOfFibers, &rq));
