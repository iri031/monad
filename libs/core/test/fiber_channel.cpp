...
namespace
{
    ...
    struct message
    {
        ...
        struct payload_t
        {
            std::uint64_t secret; ///< Message content (a secret number)
    ...
    struct channel_test_state
    {
        ...
        void push_message(std::uint64_t secret)
        {
    ...
    std::uintptr_t consumer_fiber_fn(std::uintptr_t arg0)
    {
        auto *const state = std::bit_cast<channel_test_state *>(arg0);
        while (true) {
            ...
            std::uint64_t const secret = mp->secret;
            state->messages.erase(mp->i_self);
            monad_fiber_yield(secret);
        }
        return 0;
    }
...
TEST(fiber_channel, serial_basic)
{
    ...
    monad_fiber_channel_init(
        &state.channel, monad::make_srcloc(std::source_location::current()));
    ASSERT_EQ(0, monad_fiber_channel_set_name(&state.channel, "fc_unit_test"));
    ...
    ASSERT_EQ(33, suspend_info.eval);
    ...
    ASSERT_EQ(0, monad_fiber_run(consumer_fiber, &suspend_info));
    ASSERT_EQ(MF_SUSPEND_YIELD, suspend_info.suspend_type);
    ASSERT_EQ(100, suspend_info.eval);

    // As above, for the second message
    ASSERT_EQ(consumer_fiber, monad_run_queue_try_pop(run_queue));
    ASSERT_EQ(0, monad_fiber_run(consumer_fiber, &suspend_info));
    ASSERT_EQ(MF_SUSPEND_YIELD, suspend_info.suspend_type);
    ASSERT_EQ(200, suspend_info.eval);
