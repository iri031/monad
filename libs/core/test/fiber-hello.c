...
int main(int argc, char **argv)
{
    ...
    struct monad_fiber_attr_t attr = {
        {.stack_size = 1UL << 17}}; // A 128 KiB stack
    ...
    MONAD_CONTEXT_CHECK_RESULT(monad_context_switcher_sjlj_create(&switcher));

    // Initialize the fiber, passing in our stack request
    monad_fiber_init(&hello_fiber, switcher, &attr);
    ...
    rc = monad_fiber_run(&hello_fiber, switcher, nullptr);
    assert(rc == 0);
    ...
    // We are done with this fiber
    monad_fiber_destroy(&hello_fiber);

    // Free the context switcher
    MONAD_CONTEXT_CHECK_RESULT(switcher->self_destroy(switcher));
