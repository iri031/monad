...
static const char *FIBER_STATE_NAMES[] = {
    ...
    [MONAD_FIBER_YIELD] = "YIELD",
    [MONAD_FIBER_SLEEP] = "SLEEP",
    [MONAD_FIBER_SPIN_WAIT] = "SPIN_WAIT",
    [MONAD_FIBER_RUN] = "RUN",
...
static void try_init_global_state() {
    ...
    if (!atomic_compare_exchange_strong(&g_fiber_debug_info.initialized,
                                        &is_initialized, true))
        return;
    spinlock_init(&g_fiber_debug_info.lock);
...
#define FIBER_WF(...) monad_dump_printf_field(fiber_ctx, __VA_ARGS__)
    ...
    FIBER_WF("status", "%s (%d): %lu", FIBER_STATE_NAMES[fiber->status.state],
             fiber->status.state, fiber->status.eval);
    FIBER_WF("wait_queue", "%s", MDC_PTR(fiber->wait_queue));
    FIBER_WF("rq / tfs", "%s", MDC_PTR(fiber->run_queue));
    FIBER_WF("fctx", MDC_PTR(fiber->fctx));
...
#define STACK_WF(...) monad_dump_printf_field(&stack_ctx, __VA_ARGS__)
    ...
    STACK_WF("used", "%zu KiB",
             bit_div_ceil(monad_fiber_get_used_stack_space(fiber->stack, fiber->fctx), 10));
    STACK_WF("feee", "%zu KiB",
             bit_div_ceil(monad_fiber_get_free_stack_space(fiber->stack, fiber->fctx), 10));
#undef STACK_WF
...
void monad_fiber_debug_add(monad_fiber_t *fiber) {
    ...
    spinlock_lock(&g_fiber_debug_info.lock);
    TAILQ_INSERT_TAIL(&g_fiber_debug_info.fibers, fiber, fiber_link);
    spinlock_unlock(&g_fiber_debug_info.lock);
}
...
void monad_fiber_debug_remove(monad_fiber_t *fiber) {
    ...
    spinlock_lock(&g_fiber_debug_info.lock);
    TAILQ_REMOVE(&g_fiber_debug_info.fibers, fiber, fiber_link);
    spinlock_unlock(&g_fiber_debug_info.lock);
}
...
void monad_fiber_debug_dump_all(monad_dump_ctx_t *all_fibers_ctx) {
    ...
    if (!spinlock_try_lock(&g_fiber_debug_info.lock)) {
        sleep(1);
        if (!spinlock_try_lock(&g_fiber_debug_info.lock)) {
    ...
    TAILQ_FOREACH(fiber, &g_fiber_debug_info.fibers, fiber_link) {
        ...
        case MONAD_FIBER_TYPE_THREAD:
            _monad_dump_thread_fiber_state(fiber->thread_fiber_state,
                                           &fiber_ctx);
            break;
