 ...
 *      this is what is returned if you call `monad_fiber_self()` in the context
 *      of a normal thread rather than in a "real" fiber
 *
...
[[noreturn]] static void wakeup_entrypoint(monad_fiber_transfer_t xfer_from) {
    ...
    do {
        ...
        wakeup_fiber->status.state = MONAD_FIBER_SPIN_WAIT;
        wakeup_fiber->status.eval = 0;
        ...
        wakeup_fiber->status.state = MONAD_FIBER_SLEEP; // XXX: not really
        _monad_fiber_start_switch(wakeup_fiber, thread_fiber);
    } while (true);
}
...
static void init_wakeup_fiber(monad_fiber_t *wakeup_fiber)
{
    ...
    wakeup_fiber->status.state = MONAD_FIBER_SLEEP;
    wakeup_fiber->fctx = make_fcontext(
...
static void init_thread_fiber(monad_fiber_t *thread_fiber) {
    ...
    thread_fiber->status.state = MONAD_FIBER_RUN;
