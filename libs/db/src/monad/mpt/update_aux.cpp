    ...
    AsyncIO *io_, std::optional<uint64_t> const history_len)
{
    ...
    */
    ...
    if (0 != memcmp(
        ...
        log_message(
            INFO,
            "Initialize db pool with %zu chunks in random order.",
            chunk_count);
        small_prng rand;
        ...
        log_message(
            DEBUG, "Append one chunk to fast list, id: %d", fast_offset.id);
        ...
        log_message(
            DEBUG, "Append one chunk to slow list, id: %d", slow_offset.id);
