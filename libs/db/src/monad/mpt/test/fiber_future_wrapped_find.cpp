...
namespace
{
        ...
        monad_fiber_t *fiber, monad_fiber_prio_t prio, FindArgs *find_args)
    {
        ...
        auto *switcher = monad_fiber_self()->suspended_ctx->switcher.load(
            std::memory_order_acquire);
        monad_fiber_init(fiber, switcher, &attr);
        ASSERT_EQ(
    ...
    TEST_F(OnDiskMerkleTrieGTest, single_thread_one_find_fiber)
    {
        ...
        while (true) {
            int const rc = monad_fiber_run(&fiber, nullptr, &suspend_info);
            ASSERT_EQ(rc, 0);
    ...
    TEST_F(OnDiskMerkleTrieGTest, single_thread_one_hundred_find_fibers)
    {
        ...
        ASSERT_EQ(
            monad_run_queue_create(size(one_hundred_updates), &run_queue), 0);
        for (std::size_t i = 0; auto const &[key, val] : one_hundred_updates) {
            ...
            init_find_fiber(
                &fiber, MONAD_FIBER_PRIO_HIGHEST + (ssize_t)i++, &find_args);
        ...
        while (fibers_done < size(one_hundred_updates)) {
            ...
            int const rc = monad_fiber_run(next_fiber, nullptr, &suspend_info);
            ASSERT_EQ(rc, 0);
            if (suspend_info.suspend_type == MF_SUSPEND_RETURN) {
                ASSERT_EQ(suspend_info.eval, 0);
                ++fibers_done;
