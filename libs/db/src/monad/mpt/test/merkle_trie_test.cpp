...
struct RestorePrefixTest : public OnDiskMerkleTrieGTest
{
    void restore_prefix_(NibblesView key, uint64_t version)
    {
        ...
        auto old_root = read_node_blocking(
            this->aux.io->storage_pool(),
            this->aux.get_root_offset_at_version(version));

        // Insert previous version of node at prefix
        auto res = find_blocking(this->aux, *old_root, key);
        EXPECT_TRUE(
        ...
        auto update_key = concat(
            key, node->path_nibble_view().substr(res.first.prefix_index));
        Update copy_update{
            .key = update_key,
            .value = {},
            .incarnation = false,
