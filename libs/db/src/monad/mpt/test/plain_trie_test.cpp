...
TYPED_TEST(PlainTrieTest, var_length)
{
    ...
    EXPECT_EQ(
        find_blocking(this->aux, *this->root, kv[0].first).cursor.node->value(),
        kv[0].second);
    EXPECT_EQ(
        find_blocking(this->aux, *this->root, kv[1].first).cursor.node->value(),
        kv[1].second);
    EXPECT_EQ(
        find_blocking(this->aux, *this->root, kv[2].first).cursor.node->value(),
        kv[2].second);
    EXPECT_EQ(
        find_blocking(this->aux, *this->root, kv[3].first).cursor.node->value(),
        kv[3].second);
    ...
    EXPECT_EQ(
        ...
        kv[3].second);
    EXPECT_EQ(
        find_blocking(this->aux, *this->root, kv[4].first).cursor.node->value(),
        kv[4].second);
    EXPECT_EQ(
        find_blocking(this->aux, *this->root, kv[5].first).cursor.node->value(),
        kv[5].second);
    ...
    EXPECT_EQ(
        ...
        kv[5].second);
    EXPECT_EQ(
        find_blocking(this->aux, *this->root, kv[6].first).cursor.node->value(),
        kv[6].second);
    EXPECT_EQ(
        find_blocking(this->aux, *this->root, kv[7].first).cursor.node->value(),
        kv[7].second);
...
TYPED_TEST(PlainTrieTest, delete_with_incarnation)
{
    ...
    EXPECT_EQ(
        ...
        kv[3].second);
    EXPECT_EQ(
        find_blocking(this->aux, *this->root, kv[2].first).msg,
...
TYPED_TEST(PlainTrieTest, large_values)
{
    ...
    same_upsert_to_clear_nodes_outside_cache_level();
    {
        auto [leaf_it, end_nibble, res] =
            find_blocking(this->aux, *this->root, key1);
        auto *leaf = leaf_it.node;
        EXPECT_EQ(res, find_result_msg::success);
        EXPECT_EQ(end_nibble, NibblesView{key1}.nibble_size());
    ...
    {
        auto [leaf_it, end_nibble, res] =
            find_blocking(this->aux, *this->root, key2);
        auto *leaf = leaf_it.node;
        EXPECT_EQ(res, find_result_msg::success);
        EXPECT_EQ(end_nibble, NibblesView{key2}.nibble_size());
        EXPECT_NE(leaf, nullptr);
        EXPECT_TRUE(leaf->has_value());
    ...
    same_upsert_to_clear_nodes_outside_cache_level();
    {
        monad::threadsafe_boost_fibers_promise<FindResultType> p;
        auto fut = p.get_future();
        inflight_map_t inflights;
        FiberFindRequest const req{&p, *this->root, key1};
        ...
        auto [leaf_it, end_nibble, res] = fut.get();
        auto *leaf = leaf_it.node;
        EXPECT_EQ(res, find_result_msg::success);
        EXPECT_EQ(end_nibble, NibblesView{key1}.nibble_size());
        EXPECT_NE(leaf, nullptr);
    ...
    same_upsert_to_clear_nodes_outside_cache_level();
    {
        ...
        auto fut = p.get_future();
        inflight_map_t inflights;
        FiberFindRequest const req{&p, *this->root, key2};
        find_notify_fiber_future(this->aux, inflights, req);
        while (fut.wait_for(std::chrono::seconds(0)) !=
               ::boost::fibers::future_status::ready) {
            this->aux.io->wait_until_done();
...
TYPED_TEST(PlainTrieTest, multi_level_find_blocking)
{
    ...
    auto upsert_and_find_with_prefix = [&](monad::byte_string const prefix,
        ...
        auto [begin, end_nibble, errc] =
            find_blocking(this->aux, *this->root, prefix);
        EXPECT_EQ(errc, find_result_msg::success);
        EXPECT_EQ(end_nibble, NibblesView{prefix}.nibble_size());
        EXPECT_EQ(begin.node->number_of_children(), 2);
        EXPECT_EQ(begin.node->value(), top_value);

        EXPECT_EQ(
            find_blocking(this->aux, begin, kv[0].first).cursor.node->value(),
            kv[0].second);
        EXPECT_EQ(
            find_blocking(this->aux, begin, kv[1].first).cursor.node->value(),
            kv[1].second);
        EXPECT_EQ(
            find_blocking(this->aux, begin, kv[2].first).cursor.node->value(),
            kv[2].second);
    };
