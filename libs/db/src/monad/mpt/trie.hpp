...
public:
    ...
    void print_stats(uint64_t version);
...
// deep copy of a `src_trie` to trie of `root` under prefix `dest`, where
// `src_trie` is not necessarily from the same version as `root`.
