...
size_t const statesync_upsert_message_size = 32 * 1024;
...
{
    if (net->fd == -1) {
        net->fd = socket(AF_UNIX, SOCK_STREAM, 0);
        MONAD_ASSERT(net->fd >= 0);
        struct sockaddr_un addr;
        memset(&addr, 0, sizeof(addr));
        addr.sun_family = AF_UNIX;
        strncpy(addr.sun_path, net->path.c_str(), sizeof(addr.sun_path) - 1);
        while (connect(net->fd, (sockaddr *)&addr, sizeof(addr)) != 0) {
    ...
    unsigned char const *const v2, uint64_t const size2)
{
    ...
    if (net->obuf.size() >= statesync_upsert_message_size) {
