    ...
    monad_statesync_server_context &ctx, StateDeltas const &state_deltas)
{
    ...
    std::vector<std::pair<byte_string, std::vector<byte_string>>> deleted;
    ...
    {
        decltype(ctx.deleted)::accessor it;
        if (MONAD_LIKELY(
                ctx.deleted.find(it, n - UpdateAuxImpl::VERSION_HISTORY_LEN))) {
            ctx.deleted.erase(n);
        }
    }

    MONAD_ASSERT(ctx.deleted.emplace(n, std::move(deleted)));
}
