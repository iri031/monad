...
int main(int argc, char *argv[])
{
    ...
    cli.add_flag("--on_disk", on_disk, "config TrieDb to on-disk");
    ...
    MONAD_ASSERT(!dbname_paths.empty());
    ...
    auto db = [&] -> db::TrieDb {
        if (start_block_number == 0) {
            return on_disk ? db::TrieDb{ondisk_config}
                           : db::TrieDb{std::nullopt};
        }
        ...
        if (std::filesystem::exists(dir / "accounts")) {
            ...
            return on_disk ? db::TrieDb{ondisk_config, accounts, code}
                           : db::TrieDb{std::nullopt, accounts, code};
        }
        ...
        return on_disk ? db::TrieDb{ondisk_config, ifile_stream}
                       : db::TrieDb{std::nullopt, ifile_stream};
    }();
