...
void monad_fiber_scheduler_post    (monad_fiber_scheduler_t * this, monad_fiber_task_t * task, size_t priority)
{
  MONAD_ASSERT(task != 0);
...
void monad_fiber_scheduler_dispatch(monad_fiber_scheduler_t * this, monad_fiber_task_t * task, size_t priority)
{
  ...
  while (this->thread_id_source > 0)
  {
    ...
    while (this->task_queue.size == 0u)
    {
      ...
      if (this->thread_id_source == 0)
      {
        while (this->task_queue.size > 0u)
        {
          monad_fiber_task_t * t = monad_fiber_task_queue_pop_front(&this->task_queue).task;
          t->destroy(t);
        }
        MONAD_CCALL_ASSERT(pthread_mutex_unlock(&this->mutex));
        return;
...
monad_fiber_task_t * monad_fiber_scheduler_pop_higher_priority_task(monad_fiber_scheduler_t * this, size_t priority)
{
  MONAD_CCALL_ASSERT(pthread_mutex_lock(&this->mutex));
