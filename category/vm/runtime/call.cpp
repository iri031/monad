...
namespace monad::vm::runtime
{
    namespace
    {
        std::uint32_t message_flags(
            std::uint32_t env_flags, bool static_call,
            ...
            std::int64_t remaining_block_base_gas)
        {
            ...
            auto ret_offset = (*ret_size > 0)
                                  ? ctx->get_memory_offset(ret_offset_word)
                                  : bin<0>;
            ...
            auto message = evmc_message{
                ...
                .recipient = recipient,
                .sender = sender,
                .input_data = (*args_size > 0) ? ctx->memory.data + *args_offset
                                               : nullptr,
            ...
            auto copy_size = std::min(
                static_cast<std::size_t>(*ret_size), result.output_size);
            std::copy_n(
