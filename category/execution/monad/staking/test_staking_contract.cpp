...
struct Stake : public ::testing::Test
{
    ...
    Stake()
        : machine()
        , vm()
        , dbname(create_temp_file(128)) // 128 GB
        , db{machine, mpt::OnDiskDbConfig{.dbname_paths = {dbname}}}
        // , db_cache(db)
        , tdb(db)
        , bs(tdb, vm)
        , state(bs, Incarnation{0, 0})
        , contract(state)
    {
    }
...
// {
//     // Add validator
//     auto const val = add_validator(0xdeadbeef_address,
//     ACTIVE_VALIDATOR_STAKE); EXPECT_FALSE(val.has_error());
...
//     auto const res2 =
//         change_commission(res.value().id, bad_sender,
//         200000000000000000_u256);
...
//     EXPECT_FALSE(delegate(val.value().id, del_address,
//     ACTIVE_VALIDATOR_STAKE)
//                      .has_error());
//     skip_to_next_epoch();
//     EXPECT_FALSE(syscall_reward(val.value().sign_address,
//     reward).has_error()); pull_delegator_up_to_date(val.value().id,
//     auth_address); pull_delegator_up_to_date(val.value().id, del_address);

//     auto const expected_commission = (reward * commission_percent) / 100;
//     auto const expected_delegator_reward = (reward - expected_commission) /
//     2; EXPECT_EQ(
...
//     // Create another delegator with 90% of this stake for the validator
//     pool.
//         // Otherwise, the auth delegator gets all the commission and this
//         // doesn't test anything.
//         EXPECT_FALSE(delegate(val.id, delegator, 9 * ACTIVE_VALIDATOR_STAKE)
//                          .has_error());

//     skip_to_next_epoch();

//     // change validator's commission. this won't go live until the next
//     epoch.uint256_t const new_commission = MON / 5; // 20% EXPECT_FALSE(
...
//         // auth address has 5% commission and 10% of stake pool. Note that
//         stake
//         // pool rewards are applied after the commission, so he gets two
//         rewards at
//             // 14.5% each.
//             //
//             // if the auth has stake `S` and commission `C`, both expressed
//             as percents,
...
//         pull_delegator_up_to_date(val.id, delegator);
//         EXPECT_EQ(
//             contract.vars.delegator(val.id, auth_address)
//                 .rewards()
//                 .load()
//                 .native(),
...
//         // reward before and after the boundary again. uses the new
//         commission.
//         EXPECT_FALSE(syscall_reward(val.sign_address).has_error());
//         EXPECT_FALSE(syscall_snapshot().has_error());
//         EXPECT_FALSE(syscall_reward(val.sign_address).has_error());
...
//         // commission per call (see the comment in the first epoch reward),
//         or 56 %
...
//         byte_string input;
//         input += message;
//         input += to_byte_string_view(sign_secp(message,
//         bad_secp_keys.second)); input +=
//         to_byte_string_view(sign_bls(message, good_bls_keys.second)); auto
//         const res =
...
//         input += message;
//         input += to_byte_string_view(sign_secp(message,
//         good_secp_keys.second)); input +=
//         to_byte_string_view(sign_bls(message, bad_bls_keys.second)); auto
//         const res =
//             contract.precompile_add_validator(input, address, value);
//         EXPECT_EQ(
//             res.assume_error(),
...
//     auto const res = contract.precompile_add_validator(input, address,
//     value); EXPECT_EQ(res.assume_error(), StakingError::InvalidInput);
// }
...
//         craft_add_validator_input(0xababab_address, MIN_VALIDATE_STAKE);
//     EXPECT_FALSE(
//         contract.precompile_add_validator(input, address,
//         value).has_error());
//     EXPECT_EQ(
//         contract.precompile_add_validator(input, address,
//         value).assume_error(), StakingError::ValidatorExists);
// }
...
//     auto const res = contract.precompile_add_validator(input, address,
//     value); EXPECT_EQ(res.assume_error(), StakingError::InsufficientStake);
// }
...
//         StakingError::ValueNonZero);
//     EXPECT_EQ(
//         contract.precompile_get_consensus_valset({}, {},
//         value).assume_error(), StakingError::ValueNonZero);
//     EXPECT_EQ(
//         contract.precompile_get_snapshot_valset({}, {},
//         value).assume_error(), StakingError::ValueNonZero);
//     EXPECT_EQ(
//         contract.precompile_get_execution_valset({}, {},
//         value).assume_error(), StakingError::ValueNonZero);
//     EXPECT_EQ(
...
//         ACTIVE_VALIDATOR_STAKE);
//     EXPECT_EQ(contract.vars.val_execution(1).commission().load().native(),
//     0);
//     EXPECT_EQ(contract.vars.val_execution(2).commission().load().native(),
//     0);
// }
...
//     EXPECT_EQ(
//         contract.vars.val_execution(2).get_flags(),
//         ValidatorFlagsStakeTooLow);
...
//     auto const val1 =
//         add_validator(auth_address, MIN_VALIDATE_STAKE, 0,
//         bytes32_t{0x1000});
//     EXPECT_FALSE(val1.has_error());

//     EXPECT_FALSE(delegate(val1.value().id, auth_address,
//     ACTIVE_VALIDATOR_STAKE)
//                      .has_error());
...
//     auto val2 =
//         add_validator(other_address, MIN_VALIDATE_STAKE, 0,
//         bytes32_t{0x1001});
//     EXPECT_FALSE(val2.has_error());
//     EXPECT_FALSE(delegate(val2.value().id, auth_address,
//     ACTIVE_VALIDATOR_STAKE)
//                      .has_error());
...
//     check_delegator_c_state(val2.value(), other_address, MIN_VALIDATE_STAKE,
//     0);
// }
...
//     EXPECT_FALSE(undelegate(val1.value().id, auth_address, 1,
//     50).has_error()); EXPECT_FALSE(undelegate(val2.value().id, auth_address,
//     1, 50).has_error());
// }
...
//     EXPECT_FALSE(
//         delegate(val.id, 0xabab_address,
//         ACTIVE_VALIDATOR_STAKE).has_error());
//     EXPECT_EQ(
...
//     EXPECT_FALSE(undelegate(val.id, 0xabab_address, 1,
//     ACTIVE_VALIDATOR_STAKE)
//                      .has_error());
//     EXPECT_EQ(
//         contract.vars.val_execution(val.id).get_flags(),
...
//     check_delegator_c_state(val, auth_address, ACTIVE_VALIDATOR_STAKE,
//     REWARD);
...
//     EXPECT_FALSE(
//         undelegate(val1.id, auth_address, 1,
//         MIN_VALIDATE_STAKE).has_error());
//     EXPECT_FALSE(
//         undelegate(val1.id, auth_address, 2,
//         MIN_VALIDATE_STAKE).has_error());
//     EXPECT_FALSE(
...
//         contract.vars.val_execution(val1.id).get_flags(),
//         ValidatorFlagWithdrawn | ValidatorFlagsStakeTooLow);
//     EXPECT_EQ(contract.vars.val_execution(val1.id).stake().load().native(),
//     0); EXPECT_EQ(
...
//                      .has_error());
//     EXPECT_FALSE(
//         undelegate(val2.id, other_address, 1,
//         MIN_VALIDATE_STAKE).has_error());
...
//     EXPECT_FALSE(withdraw(val1.id, auth_address, withdrawal_id).has_error());
//     EXPECT_FALSE(withdraw(val2.id, other_address,
//     withdrawal_id).has_error());
// }
...
//     EXPECT_EQ(
//         get_balance(auth_address), ACTIVE_VALIDATOR_STAKE +
//         980392156862745098);
...
//     check_delegator_c_state(val1, auth_address, ACTIVE_VALIDATOR_STAKE - 1,
//     0); check_delegator_c_state(
//         val2, auth_address, ACTIVE_VALIDATOR_STAKE - MIN_VALIDATE_STAKE - 1,
//         0);
//     check_delegator_c_state(val2, other_address, MIN_VALIDATE_STAKE, 0);
...
//     EXPECT_FALSE(
//         delegate(expected_full_valset[4], auth_address,
//         ACTIVE_VALIDATOR_STAKE)
//             .has_error());
//     EXPECT_FALSE(
//         delegate(expected_full_valset[9], auth_address,
//         ACTIVE_VALIDATOR_STAKE)
//             .has_error());
//     compare_sets(contract.vars.valset_execution, expected_full_valset);
//     skip_to_next_epoch();
...
//     EXPECT_GT(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(),
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(), REWARD);
// }
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(), REWARD + REWARD / 2);
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(), REWARD / 3);
//     EXPECT_EQ(
//         contract.vars.delegator(val.id, d0).rewards().load().native(),
//         REWARD / 3);
//     EXPECT_EQ(
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(), REWARD / 3 + REWARD / 5);
//     EXPECT_EQ(
//         contract.vars.delegator(val.id, other_address)
//             .rewards()
//             .load()
...
//     auto const d0 = 0xaaaabbbb_address;
//     auto const d1 = 0xbbbbaaaa_address;
//     EXPECT_FALSE(delegate(val.id, d0, ACTIVE_VALIDATOR_STAKE /
//     2).has_error()); EXPECT_FALSE(delegate(val.id, d0, ACTIVE_VALIDATOR_STAKE
//     / 2).has_error());

//     EXPECT_FALSE(syscall_snapshot().has_error());

//     EXPECT_FALSE(delegate(val.id, d1, ACTIVE_VALIDATOR_STAKE /
//     2).has_error()); EXPECT_FALSE(delegate(val.id, d1, ACTIVE_VALIDATOR_STAKE
//     / 2).has_error());
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(), 0);
//     EXPECT_FALSE(syscall_reward(val.sign_address).has_error());
//     EXPECT_FALSE(syscall_reward(val.sign_address).has_error());
//     EXPECT_FALSE(syscall_reward(val.sign_address).has_error());
...
//     EXPECT_EQ(contract.vars.delegator(val.id, d0).rewards().load().native(),
//     0); EXPECT_EQ(contract.vars.delegator(val.id,
//     d1).rewards().load().native(), 0);
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(), REWARD * 3 + REWARD / 2);

//     EXPECT_EQ(
//         contract.vars.delegator(val.id, d0).rewards().load().native(),
//         REWARD / 2);
//     EXPECT_EQ(contract.vars.delegator(val.id, d1).rewards().load().native(),
//     0);
...
//     acc_boundary = contract.vars.accumulated_reward_per_token(4,
//     val.id).load(); EXPECT_EQ(acc_boundary.value.native(), 0);
...
//     uint8_t const withdrawal_id{1};
//     EXPECT_FALSE(undelegate(val.id, d0, withdrawal_id,
//     ACTIVE_VALIDATOR_STAKE)
//                      .has_error());
//     EXPECT_FALSE(syscall_snapshot().has_error());
//     EXPECT_FALSE(undelegate(val.id, d1, withdrawal_id,
//     ACTIVE_VALIDATOR_STAKE)
//                      .has_error());
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(), REWARD * DELAY_WINDOW);
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         del_address).rewards().load().native(), 0);
// }
...
// {
//     // Scenario:
//     // Add a validator in epoch N. Validator is active in epoch N+1.  During
//     the
//         // the boundary between N+1 and N+2, add a delegator. Ensure the
//         delegator's
//         // accumulator is set correctly. This is an edge case because the
//         validator
...
//     // reward the validator in the boundary, so the greedy accumulator for
//     N + 2
//         // is now stale.
//         EXPECT_FALSE(syscall_reward(val.sign_address).has_error());
//     EXPECT_FALSE(syscall_reward(val.sign_address).has_error());
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(), reward_decimal_rounding);
//     EXPECT_EQ(
//         contract.vars.delegator(val.id, d0).rewards().load().native(),
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(), 2 * reward_decimal_rounding
//         + REWARD / 2 + REWARD);
...
//         EXPECT_EQ(del.rewards().load().native(), 0);
//         EXPECT_EQ(del.stake().load().native(), ACTIVE_VALIDATOR_STAKE +
//         REWARD); EXPECT_EQ(del.delta_stake().load().native(), 0);
//         EXPECT_EQ(del.next_delta_stake().load().native(), 0);
//         EXPECT_EQ(del.get_delta_epoch().native(), 0);
//         EXPECT_EQ(del.get_next_delta_epoch().native(), 0);
//     }
...
//     check_delegator_c_state(
//         val, auth_address, ACTIVE_VALIDATOR_STAKE, ((REWARD / 3) * 1));
//     check_delegator_c_state(val, d0, ACTIVE_VALIDATOR_STAKE / 2, (REWARD /
//     6));
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).stake().load().native(), ACTIVE_VALIDATOR_STAKE);
...
//     uint8_t const withdrawal_id{1};
//     EXPECT_FALSE(
//         undelegate(val.id, del_address, withdrawal_id,
//         ACTIVE_VALIDATOR_STAKE)
//             .has_error());
...
//     EXPECT_EQ(
//         contract.vars.delegator(val.id,
//         auth_address).rewards().load().native(), REWARD / 2);
//     EXPECT_EQ(
...
//     //
//     // At N, the first transaction will be an epoch change from 0 to E-1.
//     This
...
//         // execution valset during E-1 and no rewards will be issued. At
//         constexpr forkpoint
//             // M, staking begins.
...
//     res = add_validator(auth_address, MIN_VALIDATE_STAKE, 0,
//     bytes32_t{0x1002}); ASSERT_FALSE(res.has_error()); auto const val2 =
//     res.assume_value();
...
//     EXPECT_TRUE(undelegate(val1.id, d1, 1,
//     ACTIVE_VALIDATOR_STAKE).has_error());
...
//         ASSERT_FALSE(delegate(u64_be{val_id}, del, 100_u256 *
//         MON).has_error()); ASSERT_FALSE(delegate(u64_be{val_id}, del,
//         100_u256 * MON).has_error());
//     }
...
//     validators.erase(20);
//     ASSERT_FALSE(undelegate(u64_be{101}, del, 1, 200_u256 *
//     MON).has_error()); validators.erase(101);
//     ASSERT_FALSE(undelegate(u64_be{500}, del, 1, 200_u256 *
//     MON).has_error()); validators.erase(500);
...
//     // The two vectors should be equal.  This ensures that RPC style reads
//     match
...
TEST_F(Stake, bench_change_commission)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("bench_change_commission");

    for (int i = 0; i < 2000; i++) {
        auto res = add_validator(
            Address{static_cast<uint64_t>(0x1001) + static_cast<uint64_t>(i)},
            ACTIVE_VALIDATOR_STAKE,
            0,
            bytes32_t{
                static_cast<uint64_t>(0x1001) + static_cast<uint64_t>(i)});
        EXPECT_FALSE(res.has_error());
    }
    skip_to_next_epoch();
    skip_to_next_epoch();
    for (int i = 0; i < 2000; i++) {
        auto res = change_commission(
            i + 1,
            Address{static_cast<uint64_t>(0x1001) + static_cast<uint64_t>(i)},
            20);
        EXPECT_FALSE(res.has_error());
...
TEST_F(Stake, benchmark_get_epoch)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_get_epoch");

    for (int i = 0; i < 2; i++) {
        auto res = contract.precompile_get_epoch({}, {}, {});
        EXPECT_FALSE(res.has_error());
        fmt::println("Iteration {} worked", i);
        skip_to_next_epoch();
        // print entire byte_string as hex
        auto &out = res.value();
        ...
        // fmt::println("val_id={} raw bytes: 0x{}", i, oss.str());
    }
}

TEST_F(Stake, benchmark_add_validator)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_add_validator");
...
TEST_F(Stake, benchmark_get_validator)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_add_validator");
    ...
    for (int i = 1; i < 2000; i++) {
        u64_be val_id(i); // or construct however you're indexing validators
        byte_string input;
        input += to_byte_string_view(val_id.bytes);

        auto res = contract.precompile_get_validator(input, {}, {});
        EXPECT_FALSE(res.has_error());
        // unwrap the Result -> byte_string
...
TEST_F(Stake, benchmark_get_valset)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_get_valset");
...
TEST_F(Stake, benchmark_delegators)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_delegators");
    ...
    for (uint32_t i = 1; i < 2001; ++i) {
        auto const del = Address{i + 1};
        ASSERT_FALSE(delegate(i, del, 100_u256 * MON).has_error());
    }
} ///

TEST_F(Stake, benchmark_get_delegators)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_delegators");
    ...
    for (uint32_t i = 0; i < 2001; i++) {
        ...
        byte_string input;
        input += to_byte_string_view(val_id.bytes);
        auto del = Address{i + 1};
        input += to_byte_string_view(del.bytes);
        auto res = contract.precompile_get_delegator(input, {}, {});
        EXPECT_FALSE(res.has_error());
        // auto &out = res.value();
...
TEST_F(Stake, benchmark_undelegate)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_undelegate");
    ...
    for (uint32_t i = 1; i < 2000; ++i) {
        auto del2 = Address{i + 1};
        ASSERT_FALSE(delegate(i, del2, 100_u256 * MON).has_error());
    }
    skip_to_next_epoch();
    skip_to_next_epoch();

    for (uint32_t i = 1; i < 2000; ++i) {
        auto del1 = Address{i + 1};
        auto valid_withdraw_id = static_cast<uint8_t>(i % 256);
        if (valid_withdraw_id == 0) {
            continue;
        }
        ASSERT_FALSE(
            undelegate(i, del1, static_cast<uint8_t>(i % 256), 1).has_error());
    }
}

TEST_F(Stake, benchmark_withdraw)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_withdraw");
    ...
    for (uint32_t i = 1; i < 2000; ++i) {
        ...
        ASSERT_FALSE(withdraw(i, del1, static_cast<uint8_t>(i)).has_error());
    }
}

TEST_F(Stake, benchmark_get_withdraw)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_get_withdraw");
    ...
    for (uint32_t i = 1; i < 2000; ++i) {
        auto del1 = Address{i + 1};
        uint8_t valid_withdraw_id = static_cast<uint8_t>(i % 256);
        if (valid_withdraw_id == 0) {
            continue;
        }
        ...
        byte_string input;
        input += to_byte_string_view(val_id.bytes);
        input += to_byte_string_view(del1.bytes);

        unsigned char withdraw_id_bytes[1] = {valid_withdraw_id}; // <-- fix
        input += to_byte_string_view(withdraw_id_bytes);

        auto res = contract.precompile_get_withdrawal_request(input, {}, {});
        EXPECT_FALSE(res.has_error());
    }
}

TEST_F(Stake, bench_5000_fallback)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("bench_5000_fallback");
...
TEST_F(Stake, benchmark_get_delegators_for_validator_bench)
{
    monad::Stopwatch<std::chrono::milliseconds> sw(
        "benchmark_linked_list_setup");
    ...
    for (int i = 0; i < 50; i++) {
        auto res = add_validator(
            ...
            bytes32_t{
                static_cast<uint64_t>(0x1001) + static_cast<uint64_t>(i)});
        fmt::print("valid HELLO{}, delegator\n", i);

        EXPECT_FALSE(res.has_error());
        for (uint32_t j = 1; j < 500; ++j) {
            auto const del = Address{
                static_cast<uint64_t>(0x1001) + static_cast<uint64_t>(j)};
            /// fmt::print("valid HELLO{}, delegator {}\n", i, j);
            ASSERT_FALSE(delegate(i + 1, del, 100_u256 * MON).has_error());
        }
    }

} ///

TEST_F(Stake, benchmark_get_delegators_for_validator)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_linked_list");
    ...
    for (uint32_t i = 1; i < 10; ++i) {
        auto const [done, _, delegators] =
            contract.get_delegators_for_validator(
                u64_be{i}, Address{}, std::numeric_limits<uint32_t>::max());
    }
} ///

TEST_F(Stake, bench_blake3)
{
    monad::Stopwatch<std::chrono::milliseconds> sw("benchmark_linked_list");
