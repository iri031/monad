...
TEST(NodeTest, leaf)
{
    ...
    Node::UniquePtr node{make_node(0, {}, path1, true, value, {})};
...
TEST(NodeTest, leaf_single_branch)
{
    ...
    children[0].ptr = make_node(0, {}, path1, true, value, {});
    NibblesView const path2{1, 10, path.data()};
    uint16_t const mask = 1u << 0xc;
    Node::UniquePtr node{
        create_node_with_children(comp, mask, children, path2, true, 0, value)};
...
TEST(NodeTest, leaf_multiple_branches)
{
    ...
    children[0].ptr = make_node(0, {}, path1, true, value, {});
    children[1].ptr = make_node(0, {}, path1, true, value, {});
    ...
    EXPECT_EQ(node->get_mem_size(), 105);
    EXPECT_EQ(node->get_disk_size(), 93);
}
...
TEST(NodeTest, branch_node)
{
    ...
    Node::UniquePtr node{create_node_with_children(
        comp, mask, children, path2, true, 0, std::nullopt)};

    EXPECT_EQ(node->value_len, 0);
    EXPECT_EQ(node->bitpacked.data_len, 0);
    EXPECT_EQ(node->path_nibble_view(), path2);
    EXPECT_EQ(node->get_mem_size(), 94);
    EXPECT_EQ(node->get_disk_size(), 82);
}
...
TEST(NodeTest, extension_node)
{
    ...
    EXPECT_EQ(node->path_nibble_view(), path2);
    EXPECT_EQ(node->bitpacked.data_len, 0);
    EXPECT_EQ(node->get_mem_size(), 99);
    EXPECT_EQ(node->get_disk_size(), 87);
}
...
TEST(NodeTest, super_large_node)
{
    ...
    Node::UniquePtr node{make_node(0, {}, {}, true, value, {})};
