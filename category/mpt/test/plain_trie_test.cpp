...
TYPED_TEST(PlainTrieTest, var_length_trie)
{
    ...
    EXPECT_EQ(
        find_blocking(
            this->aux, {this->sm->clone(), *this->root}, kv[4].first, version)
            .first.node->value(),
        kv[4].second);
    EXPECT_EQ(
        find_blocking(
            this->aux, {this->sm->clone(), *this->root}, kv[5].first, version)
            .first.node->value(),
        kv[5].second);

    EXPECT_EQ(
        find_blocking(
            this->aux, {this->sm->clone(), *this->root}, kv[6].first, version)
            .first.node->value(),
        kv[6].second);
    EXPECT_EQ(
        find_blocking(
            this->aux, {this->sm->clone(), *this->root}, kv[7].first, version)
            .first.node->value(),
        kv[7].second);
...
TYPED_TEST(PlainTrieTest, large_values)
{
    ...
    same_upsert_to_clear_nodes_outside_cache_level();
    {
        auto [leaf_it, res] = find_blocking(
            this->aux, {this->sm->clone(), *this->root}, key1, version);
    ...
    {
        auto [leaf_it, res] = find_blocking(
            this->aux, {this->sm->clone(), *this->root}, key2, version);
        auto *leaf = leaf_it.node;
        EXPECT_EQ(res, find_result::success);
        EXPECT_NE(leaf, nullptr);
        EXPECT_TRUE(leaf->has_value());
    ...
    same_upsert_to_clear_nodes_outside_cache_level();
    {
        ...
        find_notify_fiber_future(
            this->aux, inflights, p, {this->sm->clone(), *this->root}, key1);
    ...
    same_upsert_to_clear_nodes_outside_cache_level();
    {
        ...
        inflight_map_t inflights;
        find_notify_fiber_future(
            this->aux, inflights, p, {this->sm->clone(), *this->root}, key2);
        while (fut.wait_for(std::chrono::seconds(0)) !=
               ::boost::fibers::future_status::ready) {
            this->aux.io->wait_until_done();
        }
...
TYPED_TEST(PlainTrieTest, multi_level_find_blocking)
{
    ...
    auto upsert_and_find_with_prefix = [&](monad::byte_string const prefix,
        ...
        auto [begin, errc] = find_blocking(
            this->aux, {this->sm->clone(), *this->root}, prefix, version);
        EXPECT_EQ(errc, find_result::success);
        EXPECT_EQ(begin.node->number_of_children(), 2);
        EXPECT_EQ(begin.node->value(), top_value);
        EXPECT_EQ(
            begin.machine->get_depth(), NibblesView{prefix}.nibble_size());

        for (unsigned i = 0; i <= 2; ++i) {
            auto const [it, res] =
                find_blocking(this->aux, begin, kv[i].first, version);
            ASSERT_EQ(res, find_result::success);
            EXPECT_EQ(it.node->value(), kv[i].second);
            EXPECT_EQ(
                it.machine->get_depth(),
                NibblesView{prefix + kv[i].first}.nibble_size());
        }
    };
