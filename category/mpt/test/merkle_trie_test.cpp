    ...
    }
    {
        ...
        EXPECT_EQ(
            (monad::byte_string_view{
                leaf_it.node->value_data(), leaf_it.node->value_len}),
            kv[1].second);
        auto [leaf_it2, res2, _] =
            find_blocking(this->aux, this->root, kv[2].first, version);
        EXPECT_EQ(res2, monad::mpt::find_result::success);
        EXPECT_EQ(
            (monad::byte_string_view{
                leaf_it2.node->value_data(), leaf_it2.node->value_len}),
            kv[2].second);
    }
    {
        auto const [leaf_it, res, _] =
