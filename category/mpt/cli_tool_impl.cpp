...
struct impl_t
{
    ...
    std::vector<chunk_info_archive_t> fast, slow, expire;
    ...
    void do_restore_database()
    {
        ...
        for (archive_entry *entry = nullptr;
            ...
            if (type == monad::async::storage_pool::seq) {
                if (metadata.which_list() == monad::mpt::chunk_list::free) {
                    std::stringstream ss;
                    ss << "Sequential type chunk in archive belongs to none of "
                          "fast, slow or expire list. Are you sure this "
                          "archive was generated by monad_mpt?";
                    throw std::runtime_error(ss.str());
                }
            }
        ...
        chunks.reserve(1024);
        {
            ...
            for (;;) {
                auto const *item = aux.db_metadata()->expire_list_begin();
                if (item == nullptr) {
                    break;
                }
                auto chunkid = item->index(aux.db_metadata());
        ...
        monad::mpt::detail::unsigned_20 slow_list_base_insertion_count(
            UINT32_MAX);
        monad::mpt::detail::unsigned_20 expire_list_base_insertion_count(
            UINT32_MAX);
        ...
        uint32_t expire_list_begin_index{UINT32_MAX};
        uint32_t expire_list_end_index{UINT32_MAX};
        for (auto &i : todecompress) {
            if (i.type == monad::async::storage_pool::cnv) {
                if (i.chunk_id == 0) {
                    ...
                    expire_list_end_index = old_metadata->expire_list.end;
                    break;
                }
            }
        }
        ...
        // then slow list by insertion count, expire list by insertion count,
        // with all cnv chunks to the end
        std::sort(
            todecompress.begin(),
            todecompress.end(),
            [fast_list_base_insertion_count,
             ...
             expire_list_base_insertion_count](
                ...
                if (a.type == b.type) {
                    if (a.metadata.which_list() != b.metadata.which_list()) {
                        // order: fast >> slow >> expire
                        return a.metadata.which_list() <
                               b.metadata.which_list();
                    }
                    if (a.metadata.which_list() == b.metadata.which_list()) {
                        if (a.metadata.which_list() ==
                            monad::mpt::chunk_list::fast) {
                            ...
                            a.metadata.which_list() ==
                            monad::mpt::chunk_list::slow) {
                            return (a.metadata.insertion_count() -
                                    slow_list_base_insertion_count) <
                                   (b.metadata.insertion_count() -
                                    slow_list_base_insertion_count);
        ...
        if (debug_printing) {
            // order: fast chunks, slow chunks, expire chunks, cnv chunks
            std::cerr << "Fast list:";
            auto it = todecompress.begin();
            for (; it != todecompress.end() &&
                   it->metadata.which_list() == monad::mpt::chunk_list::fast;
                 ++it) {
                std::cerr << " " << it->chunk_id;
            }
            std::cerr << "\nSlow list:";
            for (; it != todecompress.end() &&
                   it->metadata.which_list() == monad::mpt::chunk_list::slow;
                 ++it) {
                std::cerr << " " << it->chunk_id;
            }
            std::cerr << "\nExpire list:";
            for (; it != todecompress.end() &&
                 ...
                 ++it) {
                MONAD_ASSERT(
                    it->metadata.which_list() ==
                    monad::mpt::chunk_list::expire);
                std::cerr << " " << it->chunk_id;
            }
            std::cerr << std::endl;
        }
        ...
        size_t expire_chunks_inserted = 0;
        auto override_insertion_count =
            [](monad::mpt::detail::db_metadata *db,
               monad::mpt::chunk_list type,
               monad::mpt::detail::unsigned_20 initial_insertion_count) {
                MONAD_ASSERT(type != monad::mpt::chunk_list::free);
                auto g = db->hold_dirty();
                auto *i =
                    const_cast<monad::mpt::detail::db_metadata::chunk_info_t *>(
                        [type, db] {
                            if (type == monad::mpt::chunk_list::fast) {
                                return db->fast_list_begin();
                            }
                            else if (type == monad::mpt::chunk_list::slow) {
                                return db->slow_list_begin();
                            }
                            else {
                                MONAD_ASSERT(
                                    type == monad::mpt::chunk_list::expire);
                                return db->expire_list_begin();
                            }
                        }());
                i->insertion_count0_ =
        ...
        for (auto &i : todecompress) {
            if (i.type == monad::async::storage_pool::seq) {
                if (i.metadata.which_list() == monad::mpt::chunk_list::fast) {
                    aux.append(monad::mpt::chunk_list::fast, i.chunk_id);
                    ...
                    i.metadata.which_list() == monad::mpt::chunk_list::slow) {
                    aux.append(monad::mpt::chunk_list::slow, i.chunk_id);
                    ...
                    i.metadata.which_list() == monad::mpt::chunk_list::expire) {
                    aux.append(monad::mpt::chunk_list::expire, i.chunk_id);
                    if (0 == expire_chunks_inserted++) {
                        aux.modify_metadata(
                            override_insertion_count,
                            monad::mpt::chunk_list::expire,
                            expire_list_base_insertion_count);
                ...
                if (i.metadata.which_list() != monad::mpt::chunk_list::free) {
                    auto it =
        ...
        MONAD_ASSERT(
            slow_chunks_inserted + fast_chunks_inserted +
                expire_chunks_inserted +
                max_chunk_id[monad::async::storage_pool::cnv] + 1 ==
            todecompress.size());
        if (fast_chunks_inserted == 0) {
            aux.append(monad::mpt::chunk_list::fast, fast_list_begin_index);
            auto it =
        ...
        if (slow_chunks_inserted == 0) {
            aux.append(monad::mpt::chunk_list::slow, slow_list_begin_index);
            auto it =
        ...
        if (expire_chunks_inserted == 0) {
            aux.append(monad::mpt::chunk_list::expire, expire_list_begin_index);
            auto it = std::find(
                chunks.begin(), chunks.end(), expire_list_begin_index);
            MONAD_ASSERT(it != chunks.end());
            *it = UINT32_MAX;
            // override the first insertion count
            aux.modify_metadata(
        ...
        MONAD_ASSERT(
            aux.db_metadata()->expire_list.begin == expire_list_begin_index);
        MONAD_ASSERT(
            aux.db_metadata()->expire_list.end == expire_list_end_index);

        for (unsigned int &chunk : chunks) {
            if (chunk != UINT32_MAX) {
                aux.append(monad::mpt::chunk_list::free, chunk);
            }
        }
    ...
    try {
...

        {
          ...
          Capacity           Used      %  Path)";
            ...
            impl.total_used += impl.print_list_info(
                aux,
                aux.db_metadata()->expire_list_begin(),
                "Expire",
                &impl.expire);
            ...
            if (impl.rewind_database_to) {
                if (*impl.rewind_database_to <
                    ...
                    *impl.rewind_database_to >= aux.db_history_max_version()) {
                    ...
                    impl.print_list_info(
                        aux, aux.db_metadata()->expire_list_begin(), "Expire");
                    impl.print_list_info(
                        aux, aux.db_metadata()->free_list_begin(), "Free");
                    return 0;
                }
